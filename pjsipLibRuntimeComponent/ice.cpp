...
extern app_t icedemo;

void SetLog(const char* src, const char* format, ...) {
	/*char buffer[1000];

	va_list args;
	va_start(args, format);
	vsnprintf(buffer, 1000, format, va_arg(args, char*));*/
	str_output_temp.append(format);
	//va_end(args);
}
...
static void log_func(int level, const char *data, int len)
{
	...
	if(icedemo.wp_data_cb != NULL)
		icedemo.wp_data_cb((char*)data);
	//str_output_temp.append(data);
}

void setCB(void(*cb)(char*)) {
	icedemo.wp_data_cb = cb;
}
...
pj_status_t pjRC_init() {
	...
	while ((c = pj_getopt_long(1, arg, "c:n:s:t:u:p:H:L:hTFR", long_options, &opt_id)) != -1) {
		switch (c) {
		case 'c':
	...
	icedemo.opt.stun_srv = pj_str(STUN_SERVER_ADDR);
...
void icedemo_init_session(/*unsigned rolechar*/)
{
	...
	pj_ice_sess_role role = icedemo.role;
	pj_status_t status;
...
void icedemo_start_nego()
{
	...
	status = pj_ice_strans_start_ice(icedemo.icest,
		...
		icedemo.rem.cand);
	if (status != PJ_SUCCESS)
		LOG_OUTPUT("ICE failed");// icedemo_perror("Error starting ICE", status);
	else
		LOG_OUTPUT("ICE worked"); //PJ_LOG(3, (THIS_FILE, "ICE negotiation started"));
}
...
void setRoleNum(unsigned rolechar) {
	icedemo.role = (pj_tolower((pj_uint8_t)rolechar) == 'o' ?
	PJ_ICE_SESS_ROLE_CONTROLLING :
								 PJ_ICE_SESS_ROLE_CONTROLLED);
}